...
const genAI = new GoogleGenerativeAI(process.env.AI_STUDIO_GEMINI);

// Validation middleware
const validateRecommendationRequest = (req, res, next) => {
  const { tripTitle, destination, startDate, endDate, participants, budget } =
    req.body;
...
// Generate trip recommendations (temporarily without auth for testing)
router.post("/generate", validateRecommendationRequest, async (req, res) => {
  try {
    ...
    if (!process.env.AI_STUDIO_GEMINI) {
      return res.status(500).json({
        success: false,
        message: "Gemini API key not configured",
        error: "AI_STUDIO_GEMINI environment variable is missing",
      });
    }
...

{
  "tripOverview": {
    ...
    "destination": "${destination}",
    "duration": ${duration},
    "budgetPerPerson": ${Math.round(budget / participants)},
...
1. Within the specified budget of ${budget} USD total
...
7. Daily costs should add up to approximately the total budget
...
`;
    ...
    res.json({
      ...
      data: {
        ...
        userId: "test-user", // Temporarily hardcoded for testing
      },
    });
  } catch (error) {
...
router.get("/history", authenticate, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: [],
      message: "Recommendation history not implemented yet",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
